...
from ..pca_topk.utils import mask_attn_pca_topk, get_pca_components, get_sparse_mask
import methods
...
def get_sparse_transformer_and_loki_forward(args):
    def modified_forward(
        ...
        **kwargs,
        ...
        # get sparse mask for each head
        sparse_mask = get_sparse_mask(args.stride, args.c, attn_weights.shape).to(attn_weights.device)  # [batch_size, num_heads, query_len, key_len]
...
def get_sparse_transformer_forward(args):
    def modified_forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        # Compute attention for everything, mask later
...
def make_llama_attention_sparse_transformer_and_loki(args):
    print ("Modifying Llama Attention -> Sparse Transformer + Loki")
    print ("Top R:", args.top_r)
    print ("Top K:", args.top_k)
    print (f"Stride - {args.stride}")
    print (f"C - {args.c}")

    LlamaAttention.forward = get_sparse_transformer_and_loki_forward(args)

def make_llama_attention_sparse_transformer(args):
    print ("Modifying Llama Attention -> Sparse Transformer (no Loki)")
    print (f"Stride - {args.stride}")
    print (f"C - {args.c}")
