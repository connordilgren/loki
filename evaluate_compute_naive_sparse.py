...
from methods.pca_topk.attention_benchmark_naive_sparse import benchmark_attention
import json
import torch
import os
import gc
...
if __name__ == "__main__":
    os.makedirs("compute_naive_sparse", exist_ok=True)
        ...
        for prompt_length in [512, 1024, 2048]:
            for num_gen_steps in [64, 128, 256]:
                ...
                try:
                    ...
                    vanilla_filename = f"compute_naive_sparse/vanilla_prompt_{prompt_length}_gen_{num_gen_steps}.json"
                ...
                for topk in [4, 8]:
                    for topr in [4]:
                        ...
                        try:
                            ...
                            loki_filename = f"compute_naive_sparse/loki_prompt_{prompt_length}_gen_{num_gen_steps}_topk_{topk}_topr_{topr}.json"
                        ...
                        try:
                            ...
                            sparsity_filename = f"compute_naive_sparse/attention_query_key_prompt_{prompt_length}_gen_{num_gen_steps}_topk_{topk}_topr_{topr}.json"
