from methods.pca_topk.attention_benchmark_sparse_transformers import benchmark_attention
import json
import torch
    ...
    with torch.no_grad():
        for prompt_length in [512, 1024, 2048]:
            for num_gen_steps in [64, 128, 256]:
                ...
                with open(f"compute_files_sparse_transformers/vanilla_prompt_{prompt_length}_gen_{num_gen_steps}.json", "w") as f:
                ...
                for topk in [4, 8]:
                    for topr in [4]:
                        ...
                        with open(f"compute_files_sparse_transformers/loki_prompt_{prompt_length}_gen_{num_gen_steps}_topk_{topk}_topr_{topr}.json", "w") as f:
                        ...
                        for stride in [128, 512]:
                            ...
                            with open(f"compute_files_sparse_transformers/sparse_transformer_loki_prompt_{prompt_length}_gen_{num_gen_steps}_topk_{topk}_topr_{topr}_stride_{stride}.json", "w") as f:
